...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    for id in ids:
        ...
        if len(gt_bboxes2d) == 0 or len(det_bboxes2d) == 0:
            iou2d_v = torch.zeros((len(gt_bboxes2d), len(det_bboxes2d)), device='cuda')
        else:
        ...
        if len(gt_bev) == 0 or len(det_bev) == 0:
            iou_bev_v = torch.zeros((len(gt_bev), len(det_bev)), device='cuda')
        else:
        ...
        if len(gt_bboxes3d) == 0 or len(det_bboxes3d) == 0:
            iou3d_v = torch.zeros((len(gt_bboxes3d), len(det_bboxes3d)), device='cuda')
        else:
                ...
                for score_threshold in score_thresholds:
                    ...
                    for i, id in enumerate(ids):
                        ...
                        if eval_type == 'bbox_2d':
                            ...
                            if len(dc_bboxes) > 0:
                                if len(det_bboxes) == 0:
                                    ious_dc_det = np.zeros((len(dc_bboxes), 0))
                                else:
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            for j, result in enumerate(batch_results):
                ...
                # 차원 인덱싱 에러가 발생하지 않도록 명시적으로 빈 차원을 정의합니다.
                format_result_np = {}
                for k, v in format_result.items():
                    if len(v) == 0:
                        if k == 'bbox':
                            format_result_np[k] = np.zeros((0, 4))
                        elif k in ['dimensions', 'location']:
                            format_result_np[k] = np.zeros((0, 3))
                        else:
                            format_result_np[k] = np.array([])
                    else:
                        format_result_np[k] = np.array(v)
                        
                format_results[idx] = format_result_np
