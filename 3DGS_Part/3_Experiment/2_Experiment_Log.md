...
### hydrant
- 전반적으로 COLMAP 자체 결과가 좋지 않음 -> 3DGS는 30~1시간 소요 (중간에 카메라 이동이 끊기는 장면이 원인으로 추정됨)
- original은 COLMAP 1시간 이상 소요, Recon 품질이 좋지 않고, Testresults가 두 모델 모두에서 2장 나옴
- snow, de-snow 도 Recon 품질이 낮음
- LongSplat에서는 original, de-snow 이 약 20분 소요됨
### road
- 두 모델 모두에서 original과 snow 실패. 한번 더 실행함

[[Dashboard_Research|Research]] / Capstone2 / 26-04-24_Capstone_Log

## 04.24 실험 일지
### 1. 실험 진행 상황
- **대상 씬**: Sky, Stair 씬 학습 진행
- **관찰 결과**: LongSplat 기반 전처리 효과가 다른 씬에 비해 월등히 좋음.
- **분석**:
  - 앞선 4개 씬과 성능 차이가 발생하는 이유 분석 필요.
  - De-snow 씬의 경우 눈 제거 정도에 비례하여 성능 향상 확인.
  - **Unposed vs Posed**: Unposed가 전체적으로 성능이 낮은 이유 분석 중. (포즈와 가우시안 동시 학습의 한계?)
  - **Posed의 우위**: 시퀀셜 매칭(Sequential Matching) 성능의 영향 가능성 조사 필요.
### 2. 향후 계획 및 피드백
- 모든 씬 재학습(Full re-run) 검토.
- LongSplat 등 Unposed 3DGS 논문의 제안 배경 및 이론적 근거 심층 분석.
- **송호준 님 피드백**: 본 논문 제출 시 'Free Dataset'을 '기상 데이터'로 가공하여 제안하는 컨트리뷰션(Contribution) 추가 검토.